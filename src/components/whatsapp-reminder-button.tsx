...
export function WhatsAppReminderButton({ appointments, clientName, clientPhone, variant = "button" }: WhatsAppReminderButtonProps) {
    const [showButton, setShowButton] = useState(false);
    const [whatsAppLink, setWhatsAppLink] = useState('');

    useEffect(() => {
        const futureConfirmedAppointment = appointments.find(appt => appt.status === 'confirmed' && isFuture(new Date(appt.date)));

        if (futureConfirmedAppointment && clientPhone && clientName) {
    ...
    }, [appointments, clientName, clientPhone]);
    ...
    if (variant === "icon") {
        return (
            <Button variant="outline" size="icon" asChild>
                <a href={whatsAppLink} target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-white hover:bg-[#128C7E]">
                    <WhatsAppIcon className="w-5 h-5" />
                    <span className="sr-only">Enviar recordatorio por WhatsApp</span>
                </a>
            </Button>
        );
    }
